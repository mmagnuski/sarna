...
def read_set_events(filename, ignore_fields=None):
    ...
    take_fields.extend([col for col in df.columns
                        if col not in take_fields or col in ignore_fields])
...
class AutoMark(object):
    ...
    def calculate(self, raw, window=1., step=0.25, minmax=False,
                  variance=False, reduction='mean', progressbar=True):
    ...
    def _to_annotation(self, bad_bool, description='BAD_'):
        ...
        if len(groups) > 0:
            ...
            duration = ((self.window / self.sfreq) + np.diff(groups).ravel() *
                        (self.step / self.sfreq))
